...
def crossover_ex(indices, s0: Solution, s1: Solution):
    ...
    assert s0.e.shape == s1.e.shape

    # row represents the Element, and column represents the adjacency values
    ...
    for i in range(l):
        for e in [s0.e, s1.e]:
            ...
            for neighbour_idx in [idx -1, idx +1]:
                if 0 <= neighbour_idx < l:
                    neighbour_value = e[neighbour_idx]

                    adjacency_matrix[i][neighbour_value] = 1
    ...
    def shortest_adjacency_len(idx):
        ...
        for new_idx in np.where(adjacency_matrix[idx] == 1)[0]:
            idx_sum = sum(adjacency_matrix[new_idx])
            if idx_sum == 0:
                continue
    ...
    while True:
        ...
        # get elements that will be changed
        changed_elements = (adjacency_matrix[:, idx] == 1)
        ...
        # elements that had all adjacencies removed, but aren't yet in r0
        emptied_elements = np.where(np.logical_and(changed_elements, empty_rows))[0]

        if len(emptied_elements) > 0:
            emptied_elements = set(emptied_elements)

            if new_idx in emptied_elements:
                emptied_elements.remove(new_idx)

            r0 += list(emptied_elements)     
