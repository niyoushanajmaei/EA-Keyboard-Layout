...
def crossover_ex(indices, s0: Solution, s1: Solution):
    ...
    for i in range(len(s0.e)):
        ...
        for e in [s0.e, s1.e]:
            ...
            if idx -1 >= 0:
                adjacency_matrix[i].add(e[idx -1])
            if idx +1 < len(e):
                adjacency_matrix[i].add(e[idx +1])
    ...
    def shortest_adjacency_len(idx):
        ...
        for new_idx in adjacency_matrix[idx]:
            if len(adjacency_matrix[new_idx]) < best_len:
                shortest = [new_idx]
                best_len = len(adjacency_matrix[new_idx])
            elif len(adjacency_matrix[new_idx]) == best_len:
                shortest.append(new_idx)
        
        return np.random.choice(shortest)


    idx = np.random.randint(len(s0.e))
    r0  = [idx]
